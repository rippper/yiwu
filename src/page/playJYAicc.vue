  ...
  methods: {
    ...
    async updateProgress(NodeId, Time, Status) {
      ...
      this.activeNode = {
        ...
        Time,
        Status,
        Length: Math.ceil(this.duration),
        Date: date
      };
    ...
    playFunc() {
      ...
      this.myPlayer.addEventListener("error", () => {
        if (this.playType === "mp4") {
          Toast({
            message: "资源加载失败, 请检查网络坏境",
            position: "bottom"
          });
        } else {
          Toast({
            message: "资源出现问题, 请选择视频播放",
            position: "bottom"
          });
        }
      });
        ...
        if (readyState == 4) {
          ...
          this.myPlayer.addEventListener("timeupdate", () => {
            ...
            if (currentTime > 0) {
              ...
              if (this.activeNode.Status == "C") {
                ...
                /*视频未播放完毕 禁止拖拽*/
                if (
                  currentTime > this.locationTime + 4 ||
                  currentTime < this.locationTime - 4
                ) {
          ...
          this.myPlayer.addEventListener("ended", () => {
            ...
            if (this.locationTime > this.duration - 2) {
              ...
              this.updateProgress(
                this.activeNode.NodeID,
                Math.ceil(this.myPlayer.currentTime),
                this.activeNode.Status
              );
              this.nextNode();
            } else {
